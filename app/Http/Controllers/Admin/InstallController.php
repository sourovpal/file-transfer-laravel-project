...
class InstallController extends Controller
{
    ...
    public function activateApplication(Request $request)
    {
        ...
        if ($status['status'] == true) {
            ...
            $createDefaultGuest = $this->createDefaultGuest();
            if ($createDefaultGuest !== true) {
                return redirect()->back()->with('error', __('Failed to create the default guest. ' . $createDefaultGuest));
            }
     ...
     * @return bool|string
     */
    private function createDefaultGuest()
    {
        try {
            $user = new User;
            ...
            $user->status = 'active';
            $user->group = 'guest';
            $user->password = Hash::make('guest12345');
            $user->storage_total = 100000;
            $user->download_limit = 0;
            $user->email_verified_at = now();
            $user->referral_id = strtoupper(Str::random(15));
