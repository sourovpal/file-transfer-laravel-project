...
class FinanceController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('custom-plan-name', function($row){
                        $custom_status = '<span class="font-weight-bold">'.ucfirst($row["plan_name"]).'</span><br><span class="text-muted">'.number_format($row["storage_total"]).' MB</span>';
                        return $custom_status;
                    })
    ...
    {        
        if ($request->ajax()) {
            $data = Subscriber::select('subscribers.*', 'plans.plan_name', 'plans.price', 'plans.currency', 'users.name', 'users.email', 'users.profile_photo_path')->join('plans', 'plans.id', '=', 'subscribers.plan_id')->join('users', 'subscribers.user_id', '=', 'users.id')->latest()->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('actions', function($row){
                        $actionBtn = '<div>                                            
                    ...
                    ->rawColumns(['actions', 'custom-status', 'created-on', 'custom-storage', 'custom-until', 'user', 'custom-plan-name', 'custom-gateway', 'custom-frequency'])
                    ->make(true);
    ...
    public function update(Payment $id)
    {
        ...
        if ($id->status == 'completed') {
            ...
            $user = User::where('id', $id->user_id)->first();
            $plan = Plan::where('id', $id->plan_id)->first();
