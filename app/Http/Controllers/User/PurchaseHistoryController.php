...
class PurchaseHistoryController extends Controller
{
    ...
    {   
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->rawColumns(['actions', 'custom-status', 'created-on', 'custom-gateway', 'custom-amount', 'custom-plan-name', 'custom-order', 'custom-frequency', 'custom-storage'])
                    ->make(true);
    ...
    {        
        if ($request->ajax()) {
            $data = Subscriber::select('subscribers.*', 'plans.plan_name')->join('plans', 'plans.id', '=', 'subscribers.plan_id')->where('subscribers.user_id', Auth::user()->id)->latest()->get();
            return Datatables::of($data)
                    ->addIndexColumn()
                    ->addColumn('actions', function($row){
                        $actionBtn =  '<div>                                            
