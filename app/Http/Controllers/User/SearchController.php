...
class SearchController extends Controller
{
    ...
    {
        $results = Transfer::where('user_id', Auth::user()->id)->where( 'file_name', 'LIKE', '%' . $request->keyword . '%' )->orWhere( 'file_type', 'LIKE', '%' . $request->keyword . '%' )->orWhere( 'file_ext', 'LIKE', '%' . $request->keyword . '%' )->latest()->get();

        $data = Datatables::of($results)
                ->addIndexColumn()
                ->addColumn('actions', function($row){
                    $actionBtn = '<div>                                            
                        <a download href=" '. $row["transfer_url"] .'"><i class="fa-solid fa-cloud-arrow-down table-action-buttons edit-action-button" title="Download Transfer"></i></a>
                ...
                ->addColumn('private-link', function($row){
                    $private_link = ($row['signed_link']) ? '<i class="fa-solid fa-circle-check table-info-button green fs-20"></i>' : '<i class="fa-solid fa-circle-xmark red table-info-button fs-20"></i>';
                    return $private_link;
                })
                ->rawColumns(['created-on', 'private-link', 'custom-protected', 'custom-size', 'custom-format', 'custom-type', 'custom-name', 'expires-on', 'actions'])
                ->make(true);
