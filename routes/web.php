...
use App\Http\Controllers\Admin\TransferConfigController;
...
use App\Http\Controllers\User\TransferResultController;
...
Route::controller(HomeController::class)->group(function () {
    Route::get('/', 'index');
    Route::get('/download/{transfer_id}', 'download')->name('download.show');
...
Route::group(['prefix' => 'admin', 'middleware' => ['verified', '2fa.verify', 'role:admin', 'PreventBackHistory']], function() {
    ...
    // ADMIN TRANSFER MANAGEMENT ROUTES
    Route::controller(AdminTransferController::class)->group(function() {
        Route::get('/transfer/dashboard', 'index')->name('admin.transfer.dashboard');
        Route::get('/transfer/list', 'listTransfers')->name('admin.transfer.list');  
        Route::get('/transfer/list/guest', 'listGuestTransfers')->name('admin.transfer.list.guest');  
        Route::get('/transfer/{id}/show', 'show')->name('admin.transfer.show');    
        Route::post('/transfer/download', 'download');         
        Route::post('/transfer/lock', 'lock');         
        Route::post('/transfer/delete', 'delete');   
        Route::post('/transfer/list/delete', 'delete');  
    }); 
    Route::controller(UploadController::class)->group(function () {
        Route::post('/transfer/process', 'process'); 
    });
    
    // ADMIN TRANSFER CONFIGURATION ROUTES
    Route::controller(TransferConfigController::class)->group(function() {
        Route::get('/transfer/configs', 'index')->name('admin.transfer.configs');
        Route::post('/transfer/configs', 'store')->name('admin.transfer.configs.store');
    }); 
...
Route::group(['prefix' => 'user', 'middleware' => ['verified', '2fa.verify', 'role:user|admin|subscriber', 'PreventBackHistory']], function() {
    ...
    // USER TRANSFER UPLOAD ROUTES
    Route::controller(UploadController::class)->group(function () {
        Route::get('/transfer/upload', 'index')->name('user.transfer.upload');  
        Route::post('/transfer/upload', 'store');     
        Route::post('/transfer/tmp-upload', 'tmpUpload');     
        Route::delete('/transfer/tmp-delete', 'tmpDelete');     
        Route::get('/transfer/upload/{id}/show', 'show')->name('user.transfer.show');
        Route::post('/transfer/delete', 'delete');
        Route::get('/transfer/settings', 'settings');
        Route::post('/transfer/process', 'process');
    });

    // USER TRANSFER RESULTS ROUTES
    Route::controller(TransferResultController::class)->group(function () {
        Route::get('/transfer/list', 'index')->name('user.transfer.list'); 
        Route::get('/transfer/{id}/show', 'show')->name('user.transfer.list.show');
    });
