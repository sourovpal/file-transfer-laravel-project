			...
			<div class="card border-0 pt-2">
				<div class="card-body">			
					<div class="text-center">
						...
						@if ($id->payment_frequency != 'lifetime')
							<div class="text-center mb-6">
								...
								<p class="fs-14">{{ __('After successful payment, your subscription plan will be activated. To keep your subsciption plan active in coming month, please provide payments by the end of each month.') }}.</p>
								...
								<p class="text-muted fs-12 mb-4">{{ __('Total Payment Due') }}: <span class="font-weight-bold text-primary">{{ number_format((float)$total_value, 2, '.', '') }} {{ config('payment.default_currency') }}</span></p>
								<pre class="text-muted fs-12 mb-4">{{ $bank['bank_requisites'] }}</pre>
							</div>						
						@else
							<div class="text-center mb-5">
								<p class="mt-5 fs-14">{{ __('You have successfully placed order for ') }} <span class="text-info font-weight-bold">{{ $id->plan_name }}</span> {{ __(' subscription plan') }}.</p>
								<p class="fs-14">{{ __('After successful payment your lifetime subscription plan will be activated.') }}.</p>
								<p class="fs-14">{{ __('Please provide payment to our bank requisites below. Use Order ID number as payment reference') }}.</p>	
								<p class="text-muted fs-12 mb-4">{{ __('Order ID') }}: <span class="font-weight-bold text-primary">{{ $orderID }}</span></p>
								<p class="text-muted fs-12 mb-4">{{ __('Total Payment Due') }}: <span class="font-weight-bold text-primary">{{ number_format((float)$total_value, 2, '.', '') }} {{ config('payment.default_currency') }}</span></p>
								<pre class="text-muted fs-12 mb-4">{{ $bank['bank_requisites'] }}</pre>							
